<template>
  <div id="accountBill">
    ...
    <div class="container" :style="{height:!(countAll===0)?'100%':'auto'}">
      <div>
        ...
        <ul>
          ...
          <li>
            <span>下级账号：</span>
            <input placeholder="为空时查询自己" v-model="account"></li>
        </ul>
        <DateTime></DateTime>
      ...
      <!-- 数据为空时的样式 -->
      <section class="no_datas" v-if="countAll===0">
        <img class="bgimg" src="../../../../static/img/personal/account_empty.png" alt="">
        <p>暂无明细记录哦~</p>
      ...
      <article v-if="!(countAll===0)">
        <section v-for="(item,index) of dataList" :key="index">
          <p>
            <span>类别：</span>{{item.item}}</p>
          <p>
            <span>时间：</span>{{item.confirmDate.slice(0,-2)}}</p>
          <p>
            <span>金额：</span>{{item.amount}}</p>
        </section>
        <ul>
    ...
    <footer v-if="!(countAll===0)">
      <Pagination align="right" v-on:enter="submit" v-on:change="submit" :allCounts='Number(countAll)' ref='Pagination'></Pagination>
    </footer>
...
export default {
  ...
  methods: {
    ...mapActions('personal',["queryChildBill"]),
    recharge() {
      this.$router.push({
        name: "recharge",
        params: {
